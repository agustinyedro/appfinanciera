<template>
  <div class="bg-base-100 p-6 rounded-2xl shadow-md border border-base-300/50">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-lg font-semibold text-base-content">Gestionar Categorías</h2>
      <button 
        @click="showAddTag = true"
        class="bg-primary text-primary-content px-4 py-2 rounded-lg hover:bg-primary-focus transition-colors flex items-center text-sm"
      >
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <!-- Income Tags -->
      <div>
        <h3 class="font-semibold text-base-content flex items-center mb-3">
          <TrendingUp class="mr-2 h-5 w-5 text-success" />
          Ingresos
        </h3>
        <div v-if="incomeTags.length === 0" class="text-center py-6 text-base-content/60 text-sm">
        ...
        <div v-else class="space-y-2">
          ...
          >
            <div class="flex items-center">
              ...
              <span class="font-medium text-base-content text-sm">{{ tag.name }}</span>
            </div>
            <div class="flex items-center space-x-2">
              <button @click="editTag(tag)" class="text-base-content/50 hover:text-base-content"><Edit class="h-4 w-4" /></button>
              <button @click="confirmDelete(tag.id)" class="text-error/70 hover:text-error"><Trash2 class="h-4 w-4" /></button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div v-if="expenseTags.length === 0" class="text-center py-6 text-base-content/60 text-sm">
